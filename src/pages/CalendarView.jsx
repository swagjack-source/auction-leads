...
function ManageTeamModal({ onClose }) {
  ...
  return (
    ...
    >
      <div style={{ background: '#1a1d27', border: '1px solid #2a2f45', borderRadius: 14, width: '100%', maxWidth: 420, overflow: 'hidden' }}>
        {/* Header */}
        ...
        <div style={{ padding: '16px 20px', maxHeight: 300, overflowY: 'auto' }}>
          {members.length === 0 ? (
            <div style={{ color: '#555b75', fontSize: 13, textAlign: 'center', padding: '20px 0' }}>No team members yet.</div>
          ) : (
            members.map(m => (
              <div key={m.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #2a2f45' }}>
                ...
                <button onClick={() => handleDelete(m.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#555b75', padding: 4 }}>
                  <Trash2 size={14} />
                </button>
              </div>
            ))
        ...
        <div style={{ padding: '16px 20px', borderTop: '1px solid #2a2f45' }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: '#555b75', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 10 }}>Add Member</div>
          <input
            value={name}
            onChange={e => setName(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && handleAdd()}
            placeholder="Full name"
            style={{ width: '100%', background: '#0f1117', border: '1px solid #2a2f45', borderRadius: 7, padding: '8px 11px', fontSize: 13, color: '#f0f2ff', outline: 'none', marginBottom: 12 }}
          />
          {/* Color swatches */}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <button
              ...
              style={{ flex: 1, background: name.trim() ? color : '#2a2f45', border: 'none', borderRadius: 8, padding: '8px 16px', color: name.trim() ? '#fff' : '#555b75', fontSize: 13, fontWeight: 600, cursor: name.trim() ? 'pointer' : 'not-allowed', opacity: saving ? 0.7 : 1 }}
            >
...
function ConsultCard({ lead, member }) {
  ...
  const color = member?.color || '#555b75'

  return (
    <div style={{ background: '#1e2235', border: `1px solid #2a2f45`, borderLeft: `3px solid ${color}`, borderRadius: 10, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 16 }}>
      {/* Time */}
      ...
      <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
        <a
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 5, background: '#0f1117', border: '1px solid #2a2f45', borderRadius: 7, padding: '5px 10px', fontSize: 11, fontWeight: 600, color: '#8b8fa8', textDecoration: 'none', whiteSpace: 'nowrap' }}
        >
        ...
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 5, background: '#0f1117', border: '1px solid #2a2f45', borderRadius: 7, padding: '5px 10px', fontSize: 11, fontWeight: 600, color: '#8b8fa8', cursor: 'pointer' }}
        >
          <Download size={11} />
          .ics
        </button>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 13, color: '#555b75', margin: 0 }}>
          ...
          <button
            ...
            style={{ background: '#1e2235', border: '1px solid #2a2f45', borderRadius: 8, padding: '8px 14px', color: '#f0f2ff', fontSize: 13, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}
          >
            <Plus size={14} />
            Manage Team
          </button>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px' }}>
        {loading ? (
          ...
          <>
              ...
              <div style={{ marginBottom: 32 }}>
                {upcomingGroups.map(([date, items]) => (
                  <div key={date} style={{ marginBottom: 24 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: date === new Date().toISOString().slice(0, 10) ? '#6366f1' : '#555b75', textTransform: 'uppercase', letterSpacing: '0.6px', marginBottom: 10 }}>
                      {dateLabel(date)}
                    </div>
            ...
            {pastGroups.length > 0 && (
              <details>
                <summary style={{ fontSize: 11, fontWeight: 700, color: '#555b75', textTransform: 'uppercase', letterSpacing: '0.6px', cursor: 'pointer', marginBottom: 14, userSelect: 'none' }}>
